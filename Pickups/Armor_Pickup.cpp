...
#include "Armor_Pickup.h"
#include "Shooter_Online/Character/Shooter_Character.h"
#include "Shooter_Online/Shooter_Components/Buff_Component.h"


AArmor_Pickup::AArmor_Pickup()
{
    bReplicates = true;
}

void AArmor_Pickup::On_Sphere_Overlap(UPrimitiveComponent *Overlapped_Component, AActor *Other_Actor, UPrimitiveComponent *Other_Comp, int32 Other_Body_Index, bool bFrom_Sweep, const FHitResult &Sweep_Result)
{
   Super::On_Sphere_Overlap(Overlapped_Component, Other_Actor, Other_Comp, Other_Body_Index, bFrom_Sweep, Sweep_Result);
    ...
    if(Shooter_Character)
    {
        ...
        if(Buff)
        {
            Buff->Replenish_Armor(Armor_Replenish_Amount, Replenish_Armor_Time);
        }
    }
